...
function getTexture(url) {
    ...
    gl.bindTexture(gl.TEXTURE_2D, texture);
    var pixel = new Uint8Array([120, 0, 0, 255]);
...
function loadModels() {
    ...
    lightMap = getTexture("https://ncsucgclass.github.io/prog4/lightmap.gif");
        ...
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                for (whichSetVert=0; whichSetVert<numVerts; whichSetVert++) { // verts in set
                    ...
                    inputTriangles[whichSet].glTexture.push(uvToAdd[0],uvToAdd[1]);
                ...
                var str1 = "https://ncsucgclass.github.io/prog4/";
                console.log(inputTriangles[whichSet].material.texture)
                var texture_url  = str1.concat(String(inputTriangles[whichSet].material.texture));
                inputTriangles[whichSet].texObj = getTexture(texture_url);
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        void main(void) {
            ...
            vec4 textureColor = texture2D(uSampler1, vTexture);
            vec4 lightMapColor = texture2D(uSampler2, vTexture);
            
             
            if (Blinn_Phong){
                if(lightmap_flag){
                    if(uBlend){
                        gl_FragColor = vec4(textureColor.rgb * lightMapColor.rgb, textureColor.a * lightMapColor.a * uAlpha);
                    }
                    else{
                        gl_FragColor = vec4(textureColor.rgb * lightMapColor.rgb, textureColor.a * lightMapColor.a);
                    }
                }
                else{
                    if(uBlend){
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                vTextureAttribLoc = gl.getAttribLocation(shaderProgram, "aTexture"); // ptr to vertex normal attrib
                gl.enableVertexAttribArray(vTextureAttribLoc); // connect attrib to array
                
                // locate vertex uniforms
                mMatrixULoc = gl.getUniformLocation(shaderProgram, "umMatrix"); // ptr to mmat
                pvmMatrixULoc = gl.getUniformLocation(shaderProgram, "upvmMatrix"); // ptr to pvmmat
                sampler1ULoc = gl.getUniformLocation(shaderProgram, "uSampler1"); // ptr to mmat
                sampler2ULoc = gl.getUniformLocation(shaderProgram, "uSampler2"); // ptr to pvmmat
                ...
                unlitLoc = gl.getUniformLocation(shaderProgram, "lightmap_flag");
                lightupULoc = gl.getUniformLocation(shaderProgram, "uBlend");
...
function renderModels() {
    ...
    for (var whichTriSet=0; whichTriSet<numTriangleSets; whichTriSet++) {
        ...
        gl.bindTexture(gl.TEXTURE_2D,inputTriangles[whichTriSet].texObj);
        gl.uniform1i(unlitLoc, unlit);
        gl.uniform1i(sampler1ULoc, 0);
        gl.uniform1i(lightupULoc, lightup);
        gl.uniform1f(alpha, inputTriangles[whichTriSet].material.alpha);
